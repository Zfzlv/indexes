...
def processDoc():
    ...
    def realProcessDoc(row, **kwargs):
        verbose = kwargs['verbose']
        stemInnerUnicode = row['stem_inner'].decode('utf8')
        stemInnerUnicode = esIndexer.removeScriptAndStyle(stemInnerUnicode)
        stemWithoutPhoneticSymbol = pinyinPattern.sub(lambda x: phoneticSymbol.phoneticSymbol[x.group()], stemInnerUnicode)
        if stemWithoutPhoneticSymbol == stemInnerUnicode:
            return None
        else:
            stemInnerUnicode = esIndexer.removePlusAndMinus(stemWithoutPhoneticSymbol)
            return strip_tags(stemInnerUnicode).strip()
    return realProcessDoc
